...
public class CalendarView extends View implements View.OnTouchListener {
	...
	private void init() {
		LogUtils.d("看看是init先执行还是measure");
...
//		}
		...
		LogUtils.d("看看是measure先执行还是init");
...
//		canvas.drawRect(surface.weekHeight, surface.width, surface.weekHeight, surface.width, surface.cellBgPaint);
		...
		for (int i = 0; i < 42; i++) {
			...
			if (todayIndex != -1 && i == todayIndex) {
				color = mSurface.todayNumberColor;
			}
	...
	private void drawCellText(Canvas canvas, int index, String text, int color) {
		...
		float cellX = (mSurface.cellWidth * (x - 1))
				...
				/ 2f;
		canvas.drawText(text, cellX, cellY, mSurface.datePaint);
	}
	...
	private void drawDownOrSelectedBg(Canvas canvas) {
		// down and not up
		if (downDate != null) {
			drawCellBg(canvas, downIndex, mSurface.cellDownColor);
		}
		// selected bg color
		if (!selectedEndDate.before(showFirstDate)
			...
			mCalendar.setTime(curDate);
			mCalendar.add(Calendar.MONTH, -1);
			findSelectedIndex(0, curStartIndex, mCalendar, section);
			if (section[1] == -1) {
				mCalendar.setTime(curDate);
				findSelectedIndex(curStartIndex, curEndIndex, mCalendar, section);
			}
			if (section[1] == -1) {
				mCalendar.setTime(curDate);
				mCalendar.add(Calendar.MONTH, 1);
				findSelectedIndex(curEndIndex, 42, mCalendar, section);
			}
			if (section[0] == -1) {
				section[0] = 0;
			}
		...
		if (y > mSurface.monthHeight + mSurface.weekHeight) {
			...
			int n = (int) (Math
					.floor((y - (mSurface.monthHeight + mSurface.weekHeight))
							/ Float.valueOf(mSurface.cellHeight)) + 1);
			downIndex = (n - 1) * 7 + m - 1;
			...
			if (downDate != null) {
				if(isSelectMore){
					if (!completed) {
						...
						completed = true;
						//响应监听事件
						onDateClickListener.OnItemClick(selectedStartDate,selectedEndDate,downDate);
					} else {
